...
void computerMove(char** board, int N, char player) {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            if (willWin(board, N, player, i, j)) {
                board[i][j] = player;
                printf("Computer placed O at row %d, col %d (winning)\n", i+1, j+1);
                return;
            }
        }
    }
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            if (willWin(board, N, opponent, i, j)) {
                board[i][j] = player;
                printf("Computer placed O at row %d, col %d (blocking)\n", i+1, j+1);
                return;
            }
        }
    }
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            if (board[i][j] == ' ') {
                if (count == choice) {
                    board[i][j] = player;
                    printf("Computer placed O at row %d, col %d (random)\n", i+1, j+1);
                    return;
                }
                count++;
            }
